...
def parse_args():
    ...
    p.add_argument(
        "--model", default="models/best_alex.pt",
...
def build_split(data_roots: list[str], val_fraction: float, seed: int,
                rebuild: bool, base: Path):
    """
    Collect all (image, label) pairs from *data_roots*, shuffle, split into
    train/val, and symlink them into autosplit_train/ and autosplit_val/.
    ...
    if not rebuild and train_img.exists() and val_img.exists():
        ...
        return _write_yaml(base, train_img, val_img)
...
def _write_yaml(base: Path, train_img: Path, val_img: Path) -> Path:

"""
Finetune a YOLOv8-OBB model on the local buoy dataset.

Usage:
    python finetune_obb.py [options]

Requires an OBB-task base checkpoint (e.g. Ultralytics' pretrained
"yolov8n-obb.pt", or a model previously finetuned with this script) — a
plain detection checkpoint like best_alex.pt will not work here. Dataset
labels must be in YOLO-OBB format (class x1 y1 x2 y2 x3 y3 x4 y4), as
produced by labeleditor_obb.py or autoLabelGen_obb.py.

Options are documented in the argparse block below. Defaults work out of the box
with the repo layout (trainImagesZed/ + training_data/, models/best_obb.pt).
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Finetune YOLOv8-OBB on buoy dataset")
    p.add_argument(
        "--model", "-m", default="models/best_obb.pt",
        help="Path to the base OBB-task .pt checkpoint to finetune (default: models/best_obb.pt)"
    )
    p.add_argument(
        "--data-roots", "-r", nargs="+",
    ...
    p.add_argument(
        "--project", default="runs/finetune_obb",
        help="Output directory for training runs (default: runs/finetune_obb)"
    )
    p.add_argument(
        "--name", "-n", default="buoy",
...
def build_split(data_roots: list[str], val_fraction: float, seed: int,
    ...
    # Only the leading class id on each label line is read, so this works
    # unchanged for YOLO-OBB's "class x1 y1 x2 y2 x3 y3 x4 y4" lines.
...
def finetune(args):
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            ...
            "This must be an OBB-task checkpoint — e.g. Ultralytics' pretrained "
            "'yolov8n-obb.pt', or a model previously produced by this script."
        )

    print(f"\n[train] Loading model: {model_path}")
    model = YOLO(str(model_path))
    if getattr(model, "task", None) != "obb":
        print(f"[train] WARNING: checkpoint task is '{model.task}', not 'obb' — "
              "training will likely fail or produce a non-OBB model.")
    ...
    out  = base / "models" / "finetuned_obb.pt"

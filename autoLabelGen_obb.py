...
def obb_bounding_rect(corners_px):
    """Axis-aligned bounding rect (int) of a 4-point oriented box, for blur cropping."""
    xs = corners_px[:, 0]
    ys = corners_px[:, 1]
...
def run(model_path, source_type, source, out_images, out_labels,
    ...
    try:
        for frame, src_path in get_frame_iter(source_type, source):
            ...
            # Show every frame on live sources, with oriented boxes and class names overlaid
            if show:
                display = frame.copy()
                if obb is not None and len(obb) > 0:
                    corners_px_all = obb.xyxyxyxy.cpu().numpy()
                    for i in range(len(obb)):
                        pts = corners_px_all[i].astype(np.int32)
                        cls = int(obb.cls[i])
                        conf_val = float(obb.conf[i])
                        name = model.names.get(cls, str(cls))
                        cv2.polylines(display, [pts], isClosed=True, color=(0, 255, 0), thickness=2)
                        x0, y0 = pts[0]
                        cv2.putText(display, f"{name} {conf_val:.2f}", (int(x0), int(y0) - 6),
            ...
            no_detect_count = 0
            corners_px_all = obb.xyxyxyxy.cpu().numpy()
            corners_norm_all = obb.xyxyxyxyn.cpu().numpy()
            label_lines = []
            for i in range(len(obb)):
                cls = int(obb.cls[i])
                corners_px = corners_px_all[i]

                if blur_thresh > 0:
                    x1, y1, x2, y2 = obb_bounding_rect(corners_px)
                ...
                coords_str = ' '.join(f"{c:.6f}" for c in corners_norm_all[i].reshape(-1))
...
if __name__ == '__main__':
    def_model_path   = "models/best_obb.pt"
    def_source_type  = "ros"
    ...
    parser = argparse.ArgumentParser(description="Generic YOLO-OBB auto-labeling tool")
    parser.add_argument('-m', '--model', default=def_model_path,
                        help=f"Path to a YOLO-OBB .pt model (default: {def_model_path})")
